# Install and update OPENCNTX 1.8.0

OPENCNTX 1.8.0 supports Python 3.11 through 3.14. The qualified persistent
...
Do not install a second copy over an unknown installation. Download the 1.8.0
wheel and `SHA256SUMS` from the [1.8.0 release], verify the published checksum,
and run the independent manager from that wheel:

```powershell
$release = "https://github.com/CNTX-PROJECT/OPENCNTX/releases/download/v1.8.0"
Invoke-WebRequest "$release/SHA256SUMS" -OutFile SHA256SUMS
Invoke-WebRequest "$release/opencntx-1.8.0-py3-none-any.whl" -OutFile opencntx-1.8.0-py3-none-any.whl
$expected = ((Get-Content SHA256SUMS | Where-Object { $_ -match 'opencntx-1.8.0-py3-none-any.whl$' }) -split '\s+')[0]
$actual = (Get-FileHash opencntx-1.8.0-py3-none-any.whl -Algorithm SHA256).Hash.ToLowerInvariant()
if ($actual -ne $expected) { throw "OPENCNTX wheel checksum mismatch" }
pipx run --spec .\opencntx-1.8.0-py3-none-any.whl opencntx-install status
```
...
```powershell
pipx install .\opencntx-1.8.0-py3-none-any.whl
opencntx --version
opencntx-install status
```

The expected version output is `opencntx 1.8.0`.
...
pipx run --spec .\opencntx-1.8.0-py3-none-any.whl opencntx-install update `
  --artifact .\opencntx-1.8.0-py3-none-any.whl --sha256 $expected --version 1.8.0 `
...
```powershell
pipx run --spec .\opencntx-1.8.0-py3-none-any.whl opencntx-install `
...
```powershell
pipx run --spec .\opencntx-1.8.0-py3-none-any.whl opencntx-install resume
```
...
[1.8.0 release]: https://github.com/CNTX-PROJECT/OPENCNTX/releases/tag/v1.8.0
