...
This page explains the four files in the immutable `v1.2.0` GitHub Release and
...
The `v1.2.0` Production/Stable release is live and immutable. Install it from
...
- The v1.2.0 GitHub Release must contain exactly the four files named below.
...
1. `opencntx-1.2.0-py3-none-any.whl`;
2. `opencntx-1.2.0.tar.gz`;
...
python tools/release_artifacts.py verify --directory dist --expected-version 1.2.0 --expected-commit $commit --expected-tree $tree
```
...
python3 tools/release_artifacts.py verify --directory dist --expected-version 1.2.0 --expected-commit "$commit" --expected-tree "$tree"
```
...
```powershell
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.2.0-py3-none-any.whl --expected-version 1.2.0
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.2.0.tar.gz --expected-version 1.2.0
```
