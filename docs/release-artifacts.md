...
The local release helper emits exactly four v1.7.1 candidate files:

1. `opencntx-1.7.1-py3-none-any.whl`;
2. `opencntx-1.7.1.tar.gz`;
...
python tools/release_artifacts.py verify --directory dist --expected-version 1.7.1 --expected-commit $commit --expected-tree $tree
```
...
python3 tools/release_artifacts.py verify --directory dist --expected-version 1.7.1 --expected-commit "$commit" --expected-tree "$tree"
```
...
```powershell
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.7.1-py3-none-any.whl --expected-version 1.7.1
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.7.1.tar.gz --expected-version 1.7.1
```
