...
The local release helper emits exactly four v1.7.0 candidate files:

1. `opencntx-1.7.0-py3-none-any.whl`;
2. `opencntx-1.7.0.tar.gz`;
...
python tools/release_artifacts.py verify --directory dist --expected-version 1.7.0 --expected-commit $commit --expected-tree $tree
```
...
python3 tools/release_artifacts.py verify --directory dist --expected-version 1.7.0 --expected-commit "$commit" --expected-tree "$tree"
```
...
```powershell
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.7.0-py3-none-any.whl --expected-version 1.7.0
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.7.0.tar.gz --expected-version 1.7.0
```
