...
This page defines the four files for the `v1.6.1` GitHub Release and
...
The Stable release target is `v1.6.1`; published availability is established only by its
[GitHub Release](https://github.com/CNTX-PROJECT/OPENCNTX/releases/tag/v1.6.1).
...
The local release helper emits exactly four v1.6.3 candidate files:

1. `opencntx-1.6.3-py3-none-any.whl`;
2. `opencntx-1.6.3.tar.gz`;
...
python tools/release_artifacts.py verify --directory dist --expected-version 1.6.3 --expected-commit $commit --expected-tree $tree
```
...
python3 tools/release_artifacts.py verify --directory dist --expected-version 1.6.3 --expected-commit "$commit" --expected-tree "$tree"
```
...
```powershell
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.6.3-py3-none-any.whl --expected-version 1.6.3
python tools/release_artifacts.py smoke --artifact dist\opencntx-1.6.3.tar.gz --expected-version 1.6.3
```
