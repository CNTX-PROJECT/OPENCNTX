...
The instructions below target v1.6.1. Check the matching
...
Install immutable `v1.6.1` only after its exact Git tag and matching
...
Do not remove `@v1.6.1` or replace it with `main`. The pin is what binds the
...
The Stable release prints exactly `opencntx 1.6.1`.
...
introduced in v1.4.0 and retained in v1.6.1, read the
...
The immutable `v1.6.1` GitHub Release contains exactly
`opencntx-1.6.1-py3-none-any.whl`, `opencntx-1.6.1.tar.gz`, `SHA256SUMS`,
