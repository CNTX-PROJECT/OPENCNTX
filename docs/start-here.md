...
The immutable `v1.1.0` Production/Stable release is available from its exact
...
Do not remove `@v1.1.0` or replace it with `main`. The pin is what binds the
...
The Stable release prints exactly `opencntx 1.1.0`.
...
The immutable `v1.1.0` GitHub Release contains exactly
`opencntx-1.1.0-py3-none-any.whl`, `opencntx-1.1.0.tar.gz`, `SHA256SUMS`,
