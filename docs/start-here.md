...
## 2. Install the Beta release after publication

The package line is `v1.0.0rc1` and is Beta. It is publicly installable only
after both the live `v1.0.0rc1` tag and the `OPENCNTX v1.0.0rc1` GitHub Release
...
Do not remove `@v1.0.0rc1` or replace it with `main`. The pin is what binds the
...
The Beta release prints exactly `opencntx 1.0.0rc1`.
...
The `v1.0.0rc1` release exists publicly only when its live tag and GitHub Release
both exist. That Release must contain exactly
`opencntx-1.0.0rc1-py3-none-any.whl`, `opencntx-1.0.0rc1.tar.gz`, `SHA256SUMS`,
and `BUILD-RECORD.json`. Until then, all v1.0.0rc1 builds are unpublished candidates.
