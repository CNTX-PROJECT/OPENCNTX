...
The instructions below target v1.7.3. Check the matching
...
Install immutable `v1.7.3` only after its exact Git tag and matching
...
Do not remove `@v1.7.3` or replace it with `main`. The pin is what binds the
...
```powershell
git clone --branch v1.7.3 --depth 1 https://github.com/CNTX-PROJECT/OPENCNTX.git
cd OPENCNTX
...
The Stable release prints exactly `opencntx 1.7.3`.
...
introduced in v1.4.0 and retained in v1.7.3, read the
...
algorithm is delivered only through the managed update APIs described in the
[1.7.3 release scope](release-1.7.3.md). The examples above target the exact 1.7.3 release.
...
The immutable `v1.7.3` GitHub Release contains exactly
`opencntx-1.7.3-py3-none-any.whl`, `opencntx-1.7.3.tar.gz`, `SHA256SUMS`,
