...
optional.
Read the boundaries before continuing.
...
The instructions below target v1.8.2. Check the matching
...
Install immutable `v1.8.2` only after its exact Git tag and matching
...
```powershell
pipx install "https://github.com/CNTX-PROJECT/OPENCNTX/releases/download/v1.8.2/opencntx-1.8.2-py3-none-any.whl"
opencntx --version
opencntx --help
```
...
```powershell
git clone --branch v1.8.2 --depth 1 https://github.com/CNTX-PROJECT/OPENCNTX.git
cd OPENCNTX
...
The Stable release prints exactly `opencntx 1.8.2`.
...
the 1.8.2 knowledge layer, read the
[Adaptive AI workflow](adaptive-ai-workflow.md).

## Build a local knowledge index

For a project that has a main `.md` file with nested child files, build the
first-party index after the core package:
...
Review the result states and byte budget before giving any selected context to
an AI tool. See the [knowledge layer guide](knowledge-layer.md) for safe
adoption, technique cards and footer profiles.

## Upgrade or remove OPENCNTX

Use the independent read-only inventory before an upgrade. The
[1.8.2 installation guide](install-and-update.md) gives the exact managed
...
The immutable `v1.8.2` GitHub Release contains exactly
`opencntx-1.8.2-py3-none-any.whl`, `opencntx-1.8.2.tar.gz`, `SHA256SUMS`,
