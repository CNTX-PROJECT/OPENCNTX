...
_SIZES = frozenset({"SHORT", "LARGE", "MEGA"})
...
def route_project_task(
    ...
    *,
    ...
    children = tuple(_identifier(item, "child roadmap", required=True) for item in existing_child_roadmap_ids)
    ...
    if not facts.project_known:
        return base | {"action": "ANSWER_ONLY"}
    if master is None:
...
def plan_context_load(
    ...
    *,
    ...
    reduction = 0.0 if naive_bytes == 0 else round((naive_bytes - loaded_bytes) * 100 / naive_bytes, 1)
    return {
...
def decide_execution(facts: ExecutionFacts) -> dict[str, Any]:
    ...
    if facts.feedback_only:
        ...
        decision, reason, technical_mutation_allowed = "ASK_OWNER", "MATERIAL_CHOICE_REQUIRED", False
