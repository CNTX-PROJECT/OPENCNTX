...
Publication maintenance aligns current guides, separates historical navigation, retains local compatibility inputs and checks links and runtime identity. The audit also makes quick-install examples verify the published wheel hash, labels 1.8.5 as a regular limited-scope release, and distinguishes the retained preview tag from a current GitHub Release entry. It does not replace published artifacts or implement all proposed product optimizations.
...
Audit hardening: contained and bounded knowledge I/O, conditional
...
closed v1 consumers. See [release notes](docs/release-1.8.4.md) and the
[qualification roadmap](docs/roadmap-1.8.4.md). Automatic native chat-host integration is explicitly outside this release scope;
native trust and live host qualification remain open follow-up work.
...
- Align the public overview, English plan and landing-page references. Omit
...
- Publish the detailed roadmap with six phases, 24 tasks, dependencies,
...
- Made generated continuity assignment details use an English-only template
  and added a direct generated-output language regression.
- Detected quoted JSON passwords, `DB_PASSWORD` assignments and
  credential-bearing PostgreSQL URLs before optional continuity sync.
