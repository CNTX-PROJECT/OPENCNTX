...
class PublicRoadmapTests(unittest.TestCase):
    def test_current_public_navigation_uses_english_plan(self) -> None:
        self.assertFalse((ROOT / "docs/roadmap-plan.nl.md").exists())
        for name in ("README.md", "docs/README.md", "docs/roadmap.md", "site/index.html"):
            text = (ROOT / name).read_text(encoding="utf-8")
            self.assertIn("roadmap-plan.md", text)
    ...
    def test_all_tasks_and_proposals_survive_publication(self) -> None:
        ...
        self.assertIn("not software release 1.7.2", text)
        self.assertEqual(24, len(re.findall(r"^- \[ \] \*\*Status: Planned", text, re.MULTILINE)))
    ...
    def test_roadmap_markdown_links_resolve_locally(self) -> None:
        for name in ("roadmap.md", "roadmap-plan.md"):
            path = ROOT / "docs" / name
