...
class ContinuityVersionTests(unittest.TestCase):
    ...
    def upgrade(self):
        return upgrade_goal_storage(
            self.staged, expected_state_digest=self.state["state_digest"],
    ...
    def test_exact_v1_bytes_retained_new_writer_and_idempotent_migration(self) -> None:
        ...
        record_execution_checkpoint(
            self.staged, checkpoint_id="V2-CHECK", current_internal_task="VERIFY",
            next_internal_action="Verify migrated state", evidence_paths=["input.txt"],
    ...
    def test_unapproved_stale_boolean_unknown_and_tampered_versions_refused(self) -> None:
        with self.assertRaises(ContinuityError):
            upgrade_goal_storage(self.staged, expected_state_digest=self.state["state_digest"], approval="yes")
        with self.assertRaises(ContinuityError):
            upgrade_goal_storage(self.staged, expected_state_digest="a" * 64, approval=f"UPGRADE GOAL STORAGE {'a' * 64}")
        envelope = self.upgrade()
        for version in (1, 3, True, "2"):
            changed = envelope | {"format_version": version}
            changed["envelope_digest"] = _value_digest({k: v for k, v in changed.items() if k != "envelope_digest"})
    ...
    def test_shipped_envelope_schema_and_catalog_match_actual_fields(self) -> None:
        ...
        schema = json.loads(schema_root.joinpath("goal-storage-envelope-v2.schema.json").read_text())
        ...
        self.assertTrue({"goal-storage-envelope-v2.schema.json", "goal-handoff-evidence-v2.schema.json"}.issubset(catalog["schemas"]))
    ...
    def test_faulted_cutover_restores_complete_v1_and_preserves_new_version_evidence(self) -> None:
        ...
        self.upgrade()
        record_execution_checkpoint(
            self.staged, checkpoint_id="NEW-VERSION-PROOF", current_internal_task="VERIFY",
            next_internal_action="Keep new version evidence", evidence_paths=["input.txt"],
            expected_state_digest=self.state["state_digest"],
        )
        new_ledger = (self.staged / ".opencntx/continuity/history/events.jsonl").read_bytes()
        plan = build_update_preview(
            self.directory, from_version="1.4.0", to_version="R15-LOCAL",
            components=[{
                "name": "PROJECT_STATE", "active_path": str(self.root),
                "candidate_path": str(self.staged), "from_format": "1", "to_format": "2",
            }],
            target_context_version="2", target_companion_version="NONE", target_project_format="2",
            compatibility_matrix=[{"context_version": "2", "companion_version": "NONE", "project_format": "2"}],
            changelog=["Explicit goal storage envelope"], risks=["Injected cutover interruption"],
        )
        ...
        self.assertTrue(any(path.read_bytes() == new_ledger for path in preserved.rglob("events.jsonl")))
