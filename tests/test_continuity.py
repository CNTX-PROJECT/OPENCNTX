...
class ContinuityTests(unittest.TestCase):
    ...
    def test_cli_and_contract_catalog_are_machine_readable(self) -> None:
        ...
        self.assertEqual(catalog["release"], "1.7.6")
        self.assertEqual(catalog["stable_baseline"], "1.0.0")
        self.assertEqual(len(catalog["commands"]), 24)
        self.assertEqual(
            catalog["commands"][-8:],
            [
                "opencntx flow host resume",
                "opencntx flow host park",
                "opencntx flow host return",
                "opencntx-install status",
                "opencntx-install install",
                "opencntx-install update",
                "opencntx-install resume",
                "opencntx-install repair",
            ],
        )
        ...
        self.assertIn("managed-install-journal-v1.schema.json", catalog["schemas"])
